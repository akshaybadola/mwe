...
class Model:
    def __init__(self, data, args):
        ...
        with tf.variable_scope("model", reuse=tf.AUTO_REUSE):
            # The center embedding should be initialize during the training
            # process by "assign" Directly use const is not memory efficient,
            # because the constant is saved multiple times in the memory The
            # trainable here is set for not trainable for now
            self.W_amod = tf.get_variable("amod_tranform_matrix", [
                                          rel_length, embed_size])
            self.W_nsubj = tf.get_variable("nsubj_tranform_matrix", [
                                           rel_length, embed_size])
            self.W_dobj = tf.get_variable("dobj_tranform_matrix", [
                                          rel_length, embed_size])
            self.center_emb = tf.get_variable(
                "center_emb", [vocab_size, embed_size], trainable=args.center_trainable)
            self.small_amod_emb = tf.get_variable("amod_emb", [vocab_size, rel_length])
            self.small_nsubj_emb = tf.get_variable(
                "nsubj_emb", [vocab_size, rel_length])
            ...
            self.W_amod_context = tf.get_variable(
                "amod_context_tranform_matrix", [rel_length, embed_size])
            self.W_nsubj_context = tf.get_variable(
                "nsubj_context_tranform_matrix", [rel_length, embed_size])
            self.W_dobj_context = tf.get_variable(
                "dobj_context_tranform_matrix", [rel_length, embed_size])
            self.center_emb_context = tf.get_variable(
                "center_emb_context", [vocab_size, embed_size], trainable=args.center_trainable)
            self.small_amod_emb_context = tf.get_variable(
                "amod_emb_context", [vocab_size, rel_length])
            self.small_nsubj_emb_context = tf.get_variable(
                "nsubj_emb_context", [vocab_size, rel_length])
            self.small_dobj_emb_context = tf.get_variable(
                "dobj_emb_context", [vocab_size, rel_length])
            self.amod_emb_context = tf.matmul(
                self.small_amod_emb_context, self.W_amod_context)
            self.nsubj_emb_context = tf.matmul(
                self.small_nsubj_emb_context, self.W_nsubj_context)
            self.dobj_emb_context = tf.matmul(
                self.small_dobj_emb_context, self.W_dobj_context)
            ...
            self.emb_placeholder_context = tf.placeholder(
                tf.float32, [vocab_size, embed_size])
            self.emb_init_context = self.center_emb_context.assign(
                self.emb_placeholder_context)
    ...
    def add_prediction(self):
        self.accuracy_amod = tf.reduce_mean(tf.to_float(
            tf.greater(self.pred_amod, self.pred_amod_prime)))
        self.accuracy_nsubj = tf.reduce_mean(tf.to_float(
            tf.greater(self.pred_nsubj, self.pred_nsubj_prime)))
        self.accuracy_dobj = tf.reduce_mean(tf.to_float(
            tf.greater(self.pred_dobj, self.pred_dobj_prime)))
    ...
    def add_pred_amod(self):
        # center embeddings
        predicate_embedding = (1 - self.weight) *\
            tf.nn.embedding_lookup(self.center_emb, self.predicate_amod_ids) +\
            self.weight * self.restrict(
                tf.nn.embedding_lookup(
                    self.amod_emb, self.predicate_amod_ids))
        argument_embedding = (1 - self.weight) *\
            tf.nn.embedding_lookup(self.center_emb, self.argument_amod_ids) +\
            self.weight * self.restrict(
                tf.nn.embedding_lookup(
                    self.amod_emb, self.argument_amod_ids))
        argument_prime_embedding = (1 - self.weight) *\
            tf.nn.embedding_lookup(self.center_emb, self.argument_prime_amod_ids) +\
            self.weight * self.restrict(
                tf.nn.embedding_lookup(
                    self.amod_emb, self.argument_prime_amod_ids))

        # context embeddings
        predicate_embedding_context = (1 - self.weight) *\
            tf.nn.embedding_lookup(self.center_emb_context, self.predicate_amod_ids) +\
            self.weight * self.restrict(
                tf.nn.embedding_lookup(
                    self.amod_emb_context, self.predicate_amod_ids))
        argument_embedding_context = (1 - self.weight) *\
            tf.nn.embedding_lookup(self.center_emb_context, self.argument_amod_ids) +\
            self.weight * self.restrict(
                tf.nn.embedding_lookup(
                    self.amod_emb_context, self.argument_amod_ids))
        argument_prime_embedding_context = (1 - self.weight) *\
            tf.nn.embedding_lookup(self.center_emb_context, self.argument_prime_amod_ids) +\
            self.weight * self.restrict(
                tf.nn.embedding_lookup(
                    self.amod_emb_context, self.argument_prime_amod_ids))

        self.overall_predicate_amod = predicate_embedding
        self.base_embedding_handler = tf.nn.embedding_lookup(
            self.center_emb, self.predicate_amod_ids)
        self.base_embedding_context_handler = tf.nn.embedding_lookup(
            self.center_emb_context, self.predicate_amod_ids)
        ...
        self.pred_amod = tf.reduce_sum(
            tf.multiply(predicate_embedding, argument_embedding_context), 1) + \
            tf.reduce_sum(tf.multiply(predicate_embedding_context, argument_embedding), 1)
        self.pred_amod_prime = tf.reduce_sum(tf.multiply(
            predicate_embedding, argument_prime_embedding_context), 1) + \
            tf.reduce_sum(tf.multiply(predicate_embedding_context, argument_prime_embedding), 1)

    def add_pred_nsubj(self):
        predicate_embedding = (1 - self.weight) *\
            tf.nn.embedding_lookup(self.center_emb, self.predicate_nsubj_ids) +\
            self.weight * self.restrict(tf.nn.embedding_lookup(
                self.nsubj_emb, self.predicate_nsubj_ids))
        argument_embedding = (1 - self.weight) *\
            tf.nn.embedding_lookup(self.center_emb, self.argument_nsubj_ids) +\
            self.weight * self.restrict(tf.nn.embedding_lookup(
                self.nsubj_emb, self.argument_nsubj_ids))
        argument_prime_embedding = (1 - self.weight) *\
            tf.nn.embedding_lookup(self.center_emb, self.argument_prime_nsubj_ids) +\
            self.weight * self.restrict(tf.nn.embedding_lookup(
                self.nsubj_emb, self.argument_prime_nsubj_ids))

        # context embeddings
        predicate_embedding_context = (1 - self.weight) *\
            tf.nn.embedding_lookup(self.center_emb_context, self.predicate_nsubj_ids) +\
            self.weight * self.restrict(tf.nn.embedding_lookup(
                self.nsubj_emb_context, self.predicate_nsubj_ids))
        argument_embedding_context = (1 - self.weight) *\
            tf.nn.embedding_lookup(self.center_emb_context, self.argument_nsubj_ids) +\
            self.weight * self.restrict(tf.nn.embedding_lookup(
                self.nsubj_emb_context, self.argument_nsubj_ids))
        argument_prime_embedding_context = (1 - self.weight) *\
            tf.nn.embedding_lookup(self.center_emb_context, self.argument_prime_nsubj_ids) +\
            self.weight * self.restrict(tf.nn.embedding_lookup(
        ...
        self.pred_nsubj = tf.reduce_sum(
            tf.multiply(predicate_embedding, argument_embedding_context), 1) +\
            tf.reduce_sum(tf.multiply(predicate_embedding_context, argument_embedding), 1)
        self.pred_nsubj_prime = tf.reduce_sum(
            tf.multiply(predicate_embedding, argument_prime_embedding_context), 1) +\
            tf.reduce_sum(tf.multiply(predicate_embedding_context, argument_prime_embedding), 1)

    def add_pred_dobj(self):
        predicate_embedding = (1 - self.weight) *\
            tf.nn.embedding_lookup(self.center_emb, self.predicate_dobj_ids) +\
            self.weight * self.restrict(tf.nn.embedding_lookup(
                self.dobj_emb, self.predicate_dobj_ids))
        argument_embedding = (1 - self.weight) *\
            tf.nn.embedding_lookup(self.center_emb, self.argument_dobj_ids) +\
            self.weight * self.restrict(tf.nn.embedding_lookup(
                self.dobj_emb, self.argument_dobj_ids))
        argument_prime_embedding = (1 - self.weight) *\
            tf.nn.embedding_lookup(self.center_emb, self.argument_prime_dobj_ids) +\
            self.weight * self.restrict(tf.nn.embedding_lookup(
                self.dobj_emb, self.argument_prime_dobj_ids))

        # context embeddings
        predicate_embedding_context = (1 - self.weight) *\
            tf.nn.embedding_lookup(self.center_emb_context, self.predicate_dobj_ids) +\
            self.weight * self.restrict(tf.nn.embedding_lookup(
                self.dobj_emb_context, self.predicate_dobj_ids))
        argument_embedding_context = (1 - self.weight) *\
            tf.nn.embedding_lookup(self.center_emb_context, self.argument_dobj_ids) +\
            self.weight * self.restrict(tf.nn.embedding_lookup(
                self.dobj_emb_context, self.argument_dobj_ids))
        argument_prime_embedding_context = (1 - self.weight) *\
            tf.nn.embedding_lookup(self.center_emb_context, self.argument_prime_dobj_ids) +\
            self.weight * self.restrict(tf.nn.embedding_lookup(
        ...
        self.pred_dobj = tf.reduce_sum(
            tf.multiply(predicate_embedding, argument_embedding_context), 1) +\
            tf.reduce_sum(tf.multiply(predicate_embedding_context, argument_embedding), 1)
        self.pred_dobj_prime = tf.reduce_sum(
            tf.multiply(predicate_embedding, argument_prime_embedding_context), 1) +\
            tf.reduce_sum(tf.multiply(predicate_embedding_context, argument_prime_embedding), 1)

    def add_loss(self):
        num_examples = self.get_tensor_shape(self.pred_dobj, 0)
        self.prediction = tf.reshape(tf.concat(
            [self.pred_amod, self.pred_nsubj, self.pred_dobj], 0), [3*num_examples, 1] )
        self.prediction_prime = tf.reshape(tf.concat(
            [self.pred_amod_prime, self.pred_nsubj_prime, self.pred_dobj_prime], 0),
                                           [3*num_examples, 1])
        all_scores = tf.concat([self.prediction, self.prediction_prime], 1)  # [3 * num_examples, 2]
        all_labels = tf.concat(
            [tf.ones([3 * num_examples, 1]), tf.zeros([3 * num_examples, 1], 1)], 1)
        self.loss = tf.reduce_sum(tf.nn.sigmoid_cross_entropy_with_logits(
            logits=all_scores, labels=all_labels))
...
def sp10k_test_overall(sess, model, data, relation_length, restriction):
    ...
    for relation_name in relation_names:
        ...
        embeddings1 = {"base": {}, "amod": {}, "nsubj": {}, "dobj": {}}
        result1 = sess.run([model.base_embedding_handler,
                            model.base_embedding_context_handler,
                            model.amod_embedding_handler,
                            model.amod_embedding_context_handler,
        ...
        (embeddings1["base"]["h"], embeddings1["base"]["t"],
         embeddings1["amod"]["h"], embeddings1["amod"]["t"],
         embeddings1["nsubj"]["h"], embeddings1["nsubj"]["t"],
         embeddings1["dobj"]["h"], embeddings1["dobj"]["t"]) = result1
        feed_dict = {
            model.predicate_amod_ids: word_pairs[:, 1],
            model.argument_amod_ids: word_pairs[:, 1],
            model.predicate_dobj_ids: word_pairs[:, 1],
        ...
        embeddings2 = {"base": {}, "amod": {}, "nsubj": {}, "dobj": {}}
        result2 = sess.run([model.base_embedding_handler,
                            model.base_embedding_context_handler,
                            model.amod_embedding_handler,
                            model.amod_embedding_context_handler,
        ...
        (embeddings2["base"]["h"], embeddings2["base"]["t"],
         embeddings2["amod"]["h"], embeddings2["amod"]["t"],
         embeddings2["nsubj"]["h"], embeddings2["nsubj"]["t"],
         embeddings2["dobj"]["h"], embeddings2["dobj"]["t"]) = result2
        base_word_score = [1 - cosine(embeddings1["base"]["h"][j], embeddings2["base"]["t"][j])
                           for j in range(embeddings1["base"]["h"].shape[0])]
        base_corr, _ = spearmanr(gold_scores, base_word_score)
        rel_word_score = [1 - cosine(embeddings1[relation_name]["h"][j],
                                     embeddings2[relation_name]["t"][j])
                          for j in range(embeddings1[relation_name]["h"].shape[0])]
        rel_corr, _ = spearmanr(gold_scores, rel_word_score)
        print("Overall ", relation_name, "corr: %.4f  base corr:  %.4f" % (rel_corr, base_corr))
        result_list.append(rel_corr)
    with open("traverse/sp10k_result_l_%d_r_%.3f.csv " %
              (relation_length, restriction), "a+") as fout:
        fout.write("%.4f,%.4f,%.4f\n" % tuple(result_list))
...
def ws_test(sess, model, data, simlex_corps, num_cross, relation_length, restriction):
    ...
    scores = {}
    # with open("ws_gensim_result_c_%d_l_%d_r_%.3f.p " %
    #           (int(num_cross), relation_length, restriction), "wb") as fout:
    for i in range(len(simlex_corps)):
        ...
        for key in embeddings1.keys():
            ...
            embeddings1[key]["[h,t]"] = np.concatenate(
                (embeddings1[key]["h"], embeddings1[key]["t"]), axis=1)
        for key in embeddings2.keys():
            embeddings2[key]["h+t"] = embeddings2[key]["h"] + embeddings2[key]["t"]
            embeddings2[key]["[h,t]"] = np.concatenate(
                (embeddings2[key]["h"], embeddings2[key]["t"]), axis=1)
        relation_result = {}
        for relation_name in embeddings1.keys():
            types_result = {}
            for type_name in embeddings1[relation_name].keys():
                model_predition_score = [1 - cosine(embeddings1[relation_name][type_name][j],
                                                    embeddings2[relation_name][type_name][j])
                                         for j in
                                         range(embeddings1[relation_name][type_name].shape[0])]
...
def main():
    ...
    with tf.Session() as sess:
        ...
        if args.pretrain:
            ...
            sess.run(m.emb_init_context, feed_dict={
                     m.emb_placeholder_context: pretrain_context_emb})
        ...
        # center_emb, amod_emb, nsubj_emb, dobj_emb = sess.run([m.center_emb, m.amod_emb,
        # m.nsubj_emb, m.dobj_emb])
        # center_emb_context, amod_emb_context, nsubj_emb_context, dobj_emb_context =\
        # sess.run([m.center_emb_context, m.amod_emb_context, m.nsubj_emb_context, m.dobj_emb_context])
        ...
        #     for i in range(train_data.vocab_size):
        #       embed = np.concatenate(((center_emb[i, :] + center_emb_context[i, :]),
        #       (amod_emb[i, :] + amod_emb_context[i, :]), (nsubj_emb[i, :] + nsubj_emb_context[i, :]),
        #       (dobj_emb[i, :] + dobj_emb_context[i, :])), axis=0)
        #       word = train_data.id2word[i]
        #       file_.write('%s %s\n' % (word, ' '.join(map(str, embed))))
        # with open('all_embedding_concat_context.txt', 'w') as file_:
        #     for i in range(train_data.vocab_size):
        #       embed = np.concatenate((center_emb[i, :], amod_emb[i, :], nsubj_emb[i, :],
        #       dobj_emb[i, :], center_emb_context[i, :], amod_emb_context[i, :],
        #       nsubj_emb_context[i, :], dobj_emb_context[i, :]), axis=0)
        #       word = train_data.id2word[i]
        #       file_.write('%s %s\n' % (word, ' '.join(map(str, embed))))
