...
class DataLoader:
    ...
    def load_pairs_counting(self, counting_file_path):
        ...
        for key, dict_of_pairs in self.pairs_counting.items():
            ...
            values_list = numpy.array(
                list(dict_of_pairs.values())).reshape((-1, 1))
    ...
    def load_argument_sample_table(self, sample_table_path):
        ...
        for rel_type in sample_dict.keys():
            self.sample_table[rel_type] =\
                [self.word2id[word] for word, freq in sample_dict[rel_type].items()
                 if freq > fil and word in self.word2id]

    def get_sd_train_batch(self, batch_size):
        generated_pairs = {}
        for key, dict_of_pairs in self.pairs_counting.items():
            choosen_pairs_indices = numpy.random.randint(
                self.pairs_counting_list[key].shape[0], size=batch_size)
            # choosen_pairs_indices =\
            # numpy.random.choice(len(dict_of_pairs), batch_size, p=self.pairs_counting_freq)
